...
class DetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    else if (tag.contains("trend"))
      _movies = TrendingBloc.trendingMovies.results;
    else
      _movies = context.read<LikeBloc>().box.values.toList();
    return Scaffold(
      backgroundColor: Colors.red[900],
      body: SafeArea(
        child: BlocConsumer<MoviesBloc, MoviesState>(
            listener: (_, state) => context.watch<InternetBloc>().state,
            builder: (context, state) {
              if (InternetState is InternetIsDisconnected) {
                return Center(
...
/*BlocBuilder<InternetCubit,InternetState>(
        ...
        builder: (context, state) {
         if(state is InternetDisconnected){
           ToastAlertDialog.showAlert(context, "no internet ");
           return Center(child:Text("please connect to internet",
